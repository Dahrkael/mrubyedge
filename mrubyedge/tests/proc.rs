...
fn proc_closure_test() {
    ...
    if let mrubyedge::yamrb::value::RValue::Array(arr) = &result.to_rc().value {
        let arr = arr.borrow();
        assert_eq!(arr.len(), 3);

        let r1: i64 = (&arr[0]).try_into().unwrap();
        let r2: i64 = (&arr[1]).try_into().unwrap();
        let r3: i64 = (&arr[2]).try_into().unwrap();
...
fn proc_class_variable_test() {
    ...
    if let mrubyedge::yamrb::value::RValue::Array(arr) = &result.to_rc().value {
        ...
        assert!(matches!(arr[1], mrubyedge::yamrb::value::Value::Nil));
    } else {
        panic!("Expected array result");
    }
}
