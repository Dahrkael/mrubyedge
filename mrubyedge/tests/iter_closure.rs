...
fn cross_file_irep_ids_do_not_collide() {
    // Two blobs loaded into one VM must not reuse irep ids. Per-file
    // numbering used to collide, so a method returning from one file
    // captured its registers into another file's live block environment
    // (the outer env of the nested each in build_layer). That shrank the
    // captured window below the upvar index and panicked on the read.
    let build = "
    def build_layer
