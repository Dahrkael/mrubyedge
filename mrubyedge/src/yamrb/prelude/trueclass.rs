...
fn mrb_trueclass_to_s(_vm: &mut VM, _args: &[Option<Value>]) -> Result<Value, Error> {
    Ok(Value::from_rc(Rc::new(RObject::string("true".to_string()))))
}

fn mrb_trueclass_inspect(_vm: &mut VM, _args: &[Option<Value>]) -> Result<Value, Error> {
    Ok(Value::from_rc(Rc::new(RObject::string("true".to_string()))))
}

fn mrb_trueclass_and(_vm: &mut VM, args: &[Option<Value>]) -> Result<Value, Error> {
    let rhs = args[0].as_ref().unwrap().to_rc();
    Ok(Value::Bool(rhs.is_truthy()))
}

fn mrb_trueclass_or(_vm: &mut VM, _args: &[Option<Value>]) -> Result<Value, Error> {
    Ok(Value::Bool(true))
}

fn mrb_trueclass_xor(_vm: &mut VM, args: &[Option<Value>]) -> Result<Value, Error> {
    let rhs = args[0].as_ref().unwrap().to_rc();
    Ok(Value::Bool(!rhs.is_truthy()))
}
